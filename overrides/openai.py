...
def _resolve_env_api_base() -> Optional[str]:
    """Resolve the API base from environment variables.

    Preference order:
    1. OPENAI_API_BASE / OPENAI_BASE_URL / AZURE_OPENAI_API_BASE
    2. AZURE_OPENAI_ENDPOINT + AZURE_OPENAI_DEPLOYMENT (+ optional AZURE_OPENAI_API_VERSION)
    """
    for env_name in OPENAI_API_BASE_ENV_VARS:
    ...
    if azure_endpoint and azure_deployment:
        ...
        base_url = f"{endpoint}/openai/deployments/{azure_deployment}/chat/completions"
...
OPENAI_API_BASE = _resolve_env_api_base() or DEFAULT_OPENAI_API_BASE
...
class GPTAPI(BaseAPILLM):
    ...
    Args:
        ...
        key (str or List[str]): OpenAI key(s). In particular, when it
            ...
            variable $OPENAI_API_KEY (falling back to $AZURE_OPENAI_API_KEY),
            as how openai defaults to be. If it's a list, the keys will be used
            in round-robin manner. Defaults to 'ENV'.
        ...
        api_base (str): The base url of OpenAI's API. Defaults to the value
            from `OPENAI_API_BASE` / `OPENAI_BASE_URL` / `AZURE_OPENAI_API_BASE`
            environment variables when present, otherwise
        ...
        **gen_params,
        ...
        if isinstance(key, str):
            if key == "ENV":
                ...
                if env_key is None:
                    raise ValueError(
                        "OpenAI API key not found. Please set OPENAI_API_KEY or AZURE_OPENAI_API_KEY, "
                        "or provide the key explicitly when creating GPTAPI."
                    )
                self.keys = [env_key]
            else:
                self.keys = [key]
        ...
        if not self.keys:
            raise ValueError("No OpenAI API keys provided to GPTAPI.")
        ...
        if isinstance(org, str):
            ...
            self.orgs = org
        self.org_ctr = 0
        resolved_api_base = api_base or OPENAI_API_BASE
        self.url = resolved_api_base
        self.model_type = model_type
        self.proxies = proxies
        self.json_mode = json_mode
        api_type_hint = os.getenv("OPENAI_API_TYPE", "").lower()
        self._use_azure_auth_header = (
            api_type_hint == "azure"
            or bool(os.getenv("AZURE_OPENAI_API_KEY"))
            or ("openai.azure.com" in resolved_api_base.lower())
        )

    def _apply_auth_header(self, header: Dict[str, str], key: str) -> None:
        """Inject the correct authorization header depending on provider."""
        if self._use_azure_auth_header:
            header.pop("Authorization", None)
            header["api-key"] = key
        else:
            header.pop("api-key", None)
    ...
    def _chat(self, messages: List[dict], **gen_params) -> str:
        ...
        while max_num_retries < self.retry:
            ...
            self._apply_auth_header(header, key)

            if self.orgs and not self._use_azure_auth_header:
                with Lock():
                    self.org_ctr += 1
    ...
    def generate_request_data(self, model_type, messages, gen_params, json_mode=False):
        ...
        data = {}
        if model_type.lower().startswith("gpt") or model_type.lower().startswith(
            "qwen"
        ):
            if "top_k" in gen_params:
                warnings.warn(
...
class AsyncGPTAPI(AsyncBaseAPILLM):
    ...
    def __init__(
        ...
        **gen_params,
        ...
        if isinstance(key, str):
            if key == "ENV":
                ...
                if env_key is None:
                    raise ValueError(
                        "OpenAI API key not found. Please set OPENAI_API_KEY or AZURE_OPENAI_API_KEY, "
                        "or provide the key explicitly when creating AsyncGPTAPI."
                    )
                self.keys = [env_key]
            else:
                self.keys = [key]
        ...
        if not self.keys:
            raise ValueError("No OpenAI API keys provided to AsyncGPTAPI.")
    ...
    async def _stream_chat(
        self, messages: List[dict], **gen_params
